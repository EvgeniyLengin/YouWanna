...
class Manager
{
	...
	public function NormalizePhone($phone)
	{
		...
		if (!is_string($result))
			throw new \InvalidArgumentException("Normalized phone should be string!");
	...
	public function StartUserRegister($arFields)
	{
		...
		// $arFields[$phoneField] = trim(preg_replace('/[^0-9]/', '', $arFields[$phoneField]));
	...
	public function GetUsersByPhone($phone, $all = false)
	{
		...
		if ($phone_field) {
			...
			$user_tmp_phone = trim(preg_replace('/[^0-9]/', '', $phone));
			while ($u = $users->Fetch()) {
				$user_phone = $this->NormalizePhone($u[$phone_field]);

				$user_tmp_login = $u['LOGIN'];

				if ($user_phone === $phone || $user_tmp_login == $user_tmp_phone) {
					$arResult[] = array(
						"ID" => $u["ID"],
						"ACTIVE" => $u["ACTIVE"],
