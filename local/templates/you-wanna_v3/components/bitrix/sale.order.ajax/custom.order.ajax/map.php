...
if ($_REQUEST['delivery']) {
    ...
    while ($arStoreTmp = $dbList->Fetch()) {
        ...
        if ((float)$arStoreLocation['yandex_lat'] <= 0) {
            $arStoreLocation['yandex_lat'] = $arStoreTmp['GPS_N'];
        }

        if ((float)$arStoreLocation['yandex_lon'] <= 0) {
...
$showImages = (isset($_REQUEST['showImages']) && $_REQUEST['showImages'] === 'Y');
?>
<html>
<head>
            ...
            <div class="ora-storelist">
                <table id="store_table">
                    ...
                    foreach ($arStore as $val) {
                        $checked = ((string)$val['ID'] === (string)$arDefaultStore['ID']) ? 'checked' : '';
                        ?>
                        <tr class="store_row <?= $checked ?>"
                            id="row_<?= $val['ID'] ?>"
                            ...
                            <td class="<?= ($countCount !== $i) ? 'lilne' : 'last' ?>">
                                <label for="store_<?= $val['ID'] ?>">
                                    ...
                                    <div class="email"><?= GetMessage('MAP_EMAIL'); ?>:
                                        <a href="mailto:<?= htmlspecialcharsbx($val['EMAIL']) ?>"><?= htmlspecialcharsbx($val['EMAIL']) ?></a>
                                    </div>
