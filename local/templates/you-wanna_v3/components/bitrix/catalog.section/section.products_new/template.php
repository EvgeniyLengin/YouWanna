...
//NEW START 21.01.
$customFilterForCustomPages = $_GET['gt'];
//NEW OUT 21.01.
    ...
    <div id="js-ajax-nav-section-catalog"
        ...
        <? foreach ((array)$arResult['ITEMS'] as $key => $item) { ?>
            ...
            foreach($item['OFFERS'] as $offer){
                ...
                $bestCheck = $item['PROPERTIES']['BESTSELLER']['ACTIVE'];
                $newcheckoffer = $item['PROPERTIES']['NEW_PRODUCT']['VALUE_XML_ID'];
//NEW START 21.01.
                switch ($customFilterForCustomPages) {
                      ...
                      ?>
                        <a href="<?php echo $item['DETAIL_PAGE_URL'] ?>" title="<?php echo $item['NAME'] ?>">
                            <div class="element__image <?php if ( $item['PROPERTIES']['HOVER_PHOTO']['VALUE'] != 0 && !$isRecom ) { echo 'element__image--has-hover'; } ?>">
                                <?$photo = CFile::ResizeImageGet($item['DETAIL_PICTURE']['ID'] ? $item['DETAIL_PICTURE']['ID'] : $item['PREVIEW_PICTURE']['ID'], array("width"=>333, "height"=>333), BX_RESIZE_IMAGE_EXACT);?>
                                <img class="element__image--default" src="<?php echo $photo['src']; ?>" alt="">

                                <?php if ( $item['PROPERTIES']['HOVER_PHOTO']['VALUE'] != 0 && !$isRecom ) : ?>
                                    <?$hover_photo = CFile::ResizeImageGet($item['PROPERTIES']['HOVER_PHOTO']['VALUE'], array("width"=>333, "height"=>333), BX_RESIZE_IMAGE_EXACT);?>
...
//NEW START 21.01.
                            ...
                            if ($customFilterForCustomPages == "new") {
                                ...
                                <div class="form-group">
                                    <a type="button"
                                       ...
                                       data-text-red="<?= Loc::getMessage('CT_BCS_CATALOG_BTN_MESSAGE_BASKET_REDIRECT'); ?>"
                                        <?= !$firstProduct ? ' disabled="disabled"' : '' ?>
                                       data-product-id="<?= $firstProduct ?>"><?= CYouWanna::multiTranslate('Добавить в корзину', LANGUAGE_ID) ?></a>
                                </div>
...
<!--NEW START 21.01. -->
                        ...
                        <?php if ($customFilterForCustomPages == "new") { ?>
            ...
            if (!$(this).hasClass('in-cart')) {
                ...
                $.ajax({
                    ...
                    success: function () {
                        ...
                        // ЕСЛИ АВТОРИЗИРОВАН - ЗАШИФРУЕТ И ЗАПИШЕТ В поле данные корзины. init.php
                        <?php
