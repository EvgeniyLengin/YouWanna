...
foreach ( $arResult['ITEMS'] as $key => $arItem ) { // получение фотографий торговых предложений для вывода в слайдере
    ...
    if (0 !== count($arOffersSlider)) {
        ...
        foreach ($arOffersSlider as $color => $arOfferSlider) {
            foreach ((array)$arOfferSlider['PICTURES'] as $arPicture) {
                ...
                $arResult[$arItem['ID']]['SLIDER_BIG'][$color][] = $resizeImage['src'];
    ...
    foreach ((array)$arItem['OFFERS'] as $offer) {
        $arResult['ITEMS'][$key]['NEW_OFFERS'][$offer['PROPERTIES']['COLOR']['VALUE']]['NAME'] = $offer['DISPLAY_PROPERTIES']['COLOR']['DISPLAY_VALUE'];
        $arResult['ITEMS'][$key]['NEW_OFFERS'][$offer['PROPERTIES']['COLOR']['VALUE']]['ITEMS'][$offer['PROPERTIES']['SIZE']['VALUE']] = $offer;
    }
