...
<? if($arResult['ITEMS']) { ?>
    <? foreach($arResult['ITEMS'] as $key => $arItem) { ?>
        <?
            $arItem['NAME'] = CYouWanna::multiTranslate($arItem['NAME'], $arParams['LANGUAGE_ID'], false, true);
            $arItem['PREVIEW_TEXT'] = CYouWanna::multiTranslate($arItem['PREVIEW_TEXT'], $arParams['LANGUAGE_ID'], false, true);
            $arItem['PROPERTIES']['LINK_NAME']['VALUE'] = CYouWanna::multiTranslate($arItem['PROPERTIES']['LINK_NAME']['VALUE'], $arParams['LANGUAGE_ID'], false, true);
        ...
        <?if($video):?>
            ...
            <div class="yw-section yw-section--video<?if($key == 0){?> active<?}?> d-md-none" id="#page<?=$key?>">
                ...
                <video loop muted autoplay playsinline class="yw-video">
