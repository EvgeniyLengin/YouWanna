function updateUlSkuListText () {
		$('.basket-property-value-select').unbind('click', basketPropertyValueSelectHandler);
		$('.basket-property-value-select').bind('click', basketPropertyValueSelectHandler);
}

function basketPropertyValueSelectHandler() {
	$(this).siblings('.basket-item-scu-list').toggle();
}
...
$(document).click(function(event){
    if($(event.target).closest('.basket-item-property-scu-text .basket-item-property-value').length)
        return;
    $('.basket-item-property:not(.basket-item-property--color) .basket-item-scu-list').slideUp().removeClass('open');
	...
	BX.Sale.BasketComponent = {
		...
		sendRequest: function(action, data)
		{
			...
			BX.ajax({
				...
				onsuccess: BX.delegate(function(result) {
                    ...
                    if (this.params.STATIC_BASKET === 'Y') {
		...
		checkBasketItemsAnimation: function(itemData)
		{
			...
			if (this.items[itemId])
			{
				...
				{console.log(itemData);console.log(this.items[itemId]);
