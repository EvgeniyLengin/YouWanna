...
$db_props = CIBlockElement::GetProperty(4, $row['PRODUCT_ID'], array("sort" => "asc"), Array("CODE"=>"CML2_LINK"));
if($ar_props = $db_props->Fetch())
	$link = $ar_props['VALUE'];


$db_props2 = CIBlockElement::GetProperty(3, $link, array("sort" => "asc"), Array("CODE"=>"CML2_ARTICLE"));
if($ar_props2 = $db_props2->Fetch())
	$row['ARTICLE'] = $ar_props2['VALUE'];
	...
	$rowData = array(
		...
		'NAME' => CYouWanna::multiTranslate($row['NAME'], $this->arParams['LANGUAGE_ID']),
	...
	if (!empty($row['SKU_DATA']))
	{
		...
		{
            $prop['NAME'] = CYouWanna::multiTranslate($prop['NAME'], $this->arParams['LANGUAGE_ID']);
            $prop['~VALUE'] = CYouWanna::multiTranslate($prop['~VALUE'], $this->arParams['LANGUAGE_ID']);
		...
		foreach ($row['SKU_DATA'] as $skuBlock)
		{
			$skuBlockData = array(
				'ID' => $skuBlock['ID'],
				'CODE' => $skuBlock['CODE'],
				'NAME' => CYouWanna::multiTranslate($skuBlock['NAME'], $this->arParams['LANGUAGE_ID']),
				'SELECTED_VALUE' => ''
			);
			...
			foreach ($skuBlock['VALUES'] as $skuItem)
			{
				$skuValue = array(
					'ID' => $skuItem['ID'],
					'NAME' => CYouWanna::multiTranslate($skuItem['NAME'], $this->arParams['LANGUAGE_ID']),
					'SORT' => $skuItem['SORT'],
				...
				{
					$skuBlockData['SELECTED_VALUE'] = $skuValue['NAME'];
            ...
            if ($skuBlock['CODE'] === 'COLOR') {
                ...
                foreach ($skuBlockData['SKU_VALUES_LIST'] as &$item) {
                    $item['HEX'] = $arColors[$item['XML_ID']]['SKU_HEX'];
                } unset($item);
			...
			$skuBlockData['IS_COLOR'] = $isColorProperty;
...
$totalData = array(
	...
	'PRICE' => $result['allSum'],
	'ITEMS_COUNT' => $result['ORDERABLE_BASKET_ITEMS_COUNT'],
...
?>
<pre style="display: none"><?print_r($result)?></pre>