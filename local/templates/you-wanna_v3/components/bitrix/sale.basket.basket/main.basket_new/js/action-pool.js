;(function(){
	...
	{
		if (coupon == "YOU15" || coupon == "you15") {

		let currentOffers = $('.countOffers').val();
		let errorCoup = "У вас уже есть покупки. Купон недоступен."
		if (currentOffers === undefined) {
			currentOffers = 1;
			errorCoup = "Купон не доступен. Войдите в свой профиль."
		}
		if (currentOffers > 0  ) {
			let inputS = $('.form-group .form-control');
			$(inputS).closest('.form').append("<p class='warning-promo' style='padding-top:10px; padding-bottom:10px; color:red;font-weight:600;font-size:12px;'>"+errorCoup+"<p>");
			//Удаляем ошибку по промокоду
			setTimeout(function(){
			$(inputS).closest('.form').find('.warning-promo').remove();
			},4000);
			//разблокируем кнопку
			setTimeout(function(){
			$('.form-group .form-control').prop('disabled', false);
			},2000);
		} else {

			// Если не одноразовый и нет заказов = добавляем
