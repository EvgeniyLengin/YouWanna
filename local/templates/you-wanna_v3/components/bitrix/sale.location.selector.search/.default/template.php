...
<?else:?>
	...
	<div id="sls-<?=$arResult['RANDOM_TAG']?>" class="bx-sls <?if(strlen($arResult['MODE_CLASSES'])):?> <?=$arResult['MODE_CLASSES']?><?endif?>">
		...
		<div class="dropdown-block bx-ui-sls-input-block form-item-inner<?if (isset($arParams['ERROR']) && $arParams['ERROR'] !== ''){?> error<?}?>">

			<span class="dropdown-icon"></span>
			<input type="text" autocomplete="off" name="<?=$arParams['INPUT_NAME']?>" id="LOCATION" value="<?=$arResult['VALUE']?>" class="dropdown-field" placeholder="<?=Loc::getMessage('SALE_SLS_INPUT_SOME')?> ..." />
            ...
            <?if (isset($arParams['ERROR']) && $arParams['ERROR'] !== ''){?>
                <div class="error">
                    <?= $arParams['ERROR']?>
                </div>
            <?}?>
