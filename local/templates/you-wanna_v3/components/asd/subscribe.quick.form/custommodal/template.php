...
<form action="<?= POST_FORM_ACTION_URI?>" method="post" id="asd_subscribe_form">
	...
	<input type="text" class="custom-input-email-subscribe" placeholder="ВАШ E-MAIL"  name="asd_email" value="" />
	<input type="submit" name="asd_submit" id="asd_subscribe_submit" value="<?=GetMessage("ASD_SUBSCRIBEQUICK_PODPISATQSA")?>" />
	<p>НАЖИМАЯ НА КНОПКУ «ПОДПИСАТЬСЯ», ВЫ ДАЕТЕ СОГЛАСИЕ НА ОБРАБОТКУ ВАШИХ ПЕРСОНАЛЬНЫХ ДАННЫХ И СОГЛАШАЕТЕСЬ С <span> <a href="/personal/privacy-policy/">ПОЛИТИКОЙ КОНФИНДЕНЦИАЛЬНОСТИ</a> </span> </p>
