...
class SaleBasketLineComponent extends CBitrixComponent
{
	...
	protected function UpdateQuantityItem()
	{
		if (preg_match('/^[0-9]+$/', $_POST["sbblUpdateQuantityItem"]) !== 1)
			return;
		...
		$arFields = array(
			"QUANTITY" => $_POST["sbblQuantityItem"],
			);

		CSaleBasket::Update($_POST["sbblUpdateQuantityItem"], $arFields);
	}
	protected function ChangeSkuItem()
	{
		if (preg_match('/^[0-9]+$/', $_POST["sbblChangeSkuItem"]) !== 1)
			return;

		if (preg_match('/^[0-9]+$/', $_POST["sbblChangeSkuItemOld"]) !== 1)
			return;
		if (!($userFilter = $this->getUserFilter()))
			return;
	}
	...
	public function executeComponent()
	{
		...
		if (isset($_POST['sbblUpdateQuantityItem']))
			$this->UpdateQuantityItem();

		if (isset($_POST['sbblChangeSkuItem']) && isset($_POST['sbblChangeSkuItemOld']))
			$this->ChangeSkuItem();
		// prepare result
	...
	private function setImgSrc(&$arBasketItems, $arElementId, $arSku2Parent)
	{
		...
		$arImgFields = array ("PREVIEW_PICTURE", "DETAIL_PICTURE", "PROPERTY_MORE_PHOTO","PROPERTY_SIZE","PROPERTY_COLOR");
